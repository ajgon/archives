...
shared_examples_for "admin resource" do
  ...
  def incorrect_data
    ...
    unless field_with_presence_validation
      ...
      binary_columns = model.columns.find_all {|c| c.type == :binary}.collect {|c| c.name.to_s}
      build(model.to_s.underscore.to_sym, field_with_presence_validation => '').attributes.except(*(['id', 'created_at', 'updated_at'] + binary_columns))
    end
  end
  ...
  def correct_data
    (model = described_class.extract_resource).should be_a_descendant_of(ActiveRecord::Base)
    binary_columns = model.columns.find_all {|c| c.type == :binary}.collect {|c| c.name.to_s}
    build(model.to_s.underscore.to_sym).attributes.except(*(['id', 'created_at', 'updated_at'] + binary_columns))
  end
  ...
  describe '#index' do
    ...
    it 'should render base object table' do
      ...
      all('thead tr th').size.should eq(model.columns.size - model.columns.find_all {|c| c.type == :binary}.size + 1)
    end
    ...
    describe 'dataTables' do
      ...
      let(:check_response_for_dataTables) do
        ...
        results = @model.all(@expected_behaviour.merge(:offset => @params[:iDisplayStart], :limit => iDisplayLength))
        ...
        results.each.with_index do |result, r|
          @columns.each.with_index do |column, c|
            json['aaData'][r][c.to_s].to_s.should eq(result.send(column).to_s)
          end
      ...
      it 'should contain button with options for each row of dataTable' do
        ...
        json['aaData'].each do |result|
          id = result['0']
          result[last_column_index].should match(Regexp.new("('|\")/admin/[^\"']+/#{id}/edit('|\")"))
          result[last_column_index].should match(Regexp.new("('|\")/admin/[^\"']+/#{id}('|\")"))
          result[last_column_index].should match(/data-method=('|")delete('|")/)
      ...
      it 'should filter search results' do
        ...
        @params[:sSearch] = search = model.all.collect do |m|
          column_name = model.columns.find {|c| c.type == :string}.name
          m.send(column_name) unless m.send(column_name).blank?
        end.first

        @expected_behaviour = {:conditions => "`" + @columns.join("` LIKE '%#{search}%' OR `") + "` LIKE '%#{search}%'"}
      ...
      it 'should sort by third column ascending and then by first column descending' do
        ...
        @expected_behaviour = {:order => "`#{@columns[2]}` ASC, `#{@columns[0]}` DESC"}
        check_response_for_dataTables
      end
    end
  ...
  context 'with existing resource' do
    ...
    describe "#update" do
      subject { put :update, :id => resource.id, :user => attr }
