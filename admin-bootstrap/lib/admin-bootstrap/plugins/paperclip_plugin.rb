class AdminBootstrap::Plugins::PaperclipPlugin < AdminBootstrap::Plugins::Base
  ...
  default :paperclip do |model|
    model.attachment_definitions.keys.each do |column|
      model.admin_column column, (model.admin_column(column).merge(:paperclip => true))
    end
  end
  ...
  option :paperclip do |value, model, column|
    if value
      ...
      if attachment
        model.admin_column column, (model.admin_column(column).merge(:after => ((column = column.to_s) + '_updated_at').to_sym))
