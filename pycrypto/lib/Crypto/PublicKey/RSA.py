...
from Crypto.PublicKey import _RSA, _slowmath, pubkey, tables
from Crypto import Random
from Crypto.Hash import MD5

from Crypto.Util.asn1 import DerObject, DerSequence, BER
...
class RSAImplementation(object):
	...
	def _importKeyBER(self, externKey):
		ber = BER(externKey).decode()
		if len(ber) == 9 and ber[0] == 0:
			del ber[6:8]
			del ber[0]
			return self.construct(ber[:])

	def importKey(self, externKey, password=None):
		...
		if externKey.startswith('-----'):
			...
			content = "\n".join(externKey.replace(" ",'').split("\n")[1:-2]).split("\n\n")
			if len(content) == 1:
				data = content[0]
				headers = []
			else:
				headers, data = content[0], content[1]
				headers = dict((h.split(':')[0].lower(), h.split(':')[1].strip()) for h in headers.split())
			data = binascii.a2b_base64(''.join(data))
			if 'proc-type' not in headers:
				return self._importKeyDER(data)
			encryption_type, saltstr = headers['dek-info'].split(',')
			if encryption_type not in tables.CIPHER:
				raise ValueError('Unknown private key cipher "%s"' % encryption_type)
			if password is None:
				raise ValueError('Private key file is encrypted')
			cipher = tables.CIPHER[encryption_type]['cipher']
			keysize = tables.CIPHER[encryption_type]['keysize']
			mode = tables.CIPHER[encryption_type]['mode']
			salt = binascii.unhexlify(saltstr)
			key = self.__generate_key_bytes(MD5, salt, password, keysize)
			return self._importKeyBER(cipher.new(key, mode, salt).decrypt(data))
		if externKey[0]=='\x30':
			# This is probably a DER encoded key
			return self._importKeyDER(externKey)
		raise ValueError("RSA key format is not supported")
		
	def __generate_key_bytes(self, hashclass, salt, key, nbytes):
		keydata = ''
		digest = ''
		if len(salt) > 8:
			salt = salt[:8]
